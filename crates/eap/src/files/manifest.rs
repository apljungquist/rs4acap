...
impl Manifest {
    pub(crate) fn new(
        ...
        let mut manifest = manifest.as_value().clone();
...
mod tests {
    ...
    fn architecture_of(manifest: &Manifest) -> Option<&str> {
        manifest
            .as_value()
            .get("acapPackageConf")?
            .get("setup")?
    ...
    fn architecture_is_not_added_before_schema_version_1_3() {
        ...
        .unwrap();
        assert_eq!(architecture_of(&manifest), None);
    }
